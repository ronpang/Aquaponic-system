...
secrets = {
    'aio_username' : 'XXXXXXXXXXXXXXXXX',   ### Wirte Username here ###
    'aio_key' : 'XXXXXXXXXXXXXXXXXXXXXXXX'  ### Write Active Key here ###
    }
...
water_alarm = digitalio.DigitalInOut(board.GP1)
water_alarm.direction = digitalio.Direction.INPUT
...
sled = digitalio.DigitalInOut(board.GP3)
...
class data_manage:
    ...
    average = 0 
    total = 0
    ph_min = 20 #error value from the module
    orp_min = 40
    orp_cal = 0
    ...
    def calculation (self,choice):
        if choice == 'PH':
            if len(self.P_data) == self.sample:
                self.total = sum(self.P_data)
                self.average = int(self.total/len(self.P_data)/64)
